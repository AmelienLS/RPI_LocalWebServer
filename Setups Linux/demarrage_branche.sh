#!/usr/bin/env bash
# ==============================================================================
# demarrage_branche.sh
# Permet de choisir une branche GitHub puis lance RPI_LocalWebServer.
#
# Compatible : Ubuntu 20.04+  |  Fedora (standard + Silverblue/Kinoite)
#
# Usage :
#   bash demarrage_branche.sh
#
...
select_branch() {
    ...
    local raw_branches
    raw_branches="$(git ls-remote --heads "$REPO_URL" 2>/dev/null \
        | awk -F'refs/heads/' '{print $2}' \
        | sort)" \
        || die "Impossible de contacter le dépôt. Vérifiez votre connexion internet."

    if [[ -z "$raw_branches" ]]; then
        die "Aucune branche trouvée dans le dépôt ($REPO_URL)."
    fi
    ...
    printf '\n'
    local selection
    read -rp "[?] Branche à utiliser [$DEFAULT_BRANCH] : " selection || true
    BRANCH="${selection:-$DEFAULT_BRANCH}"

    # Vérifier que la branche saisie existe
    if ! grep -qx "$BRANCH" <<< "$raw_branches"; then
        die "Branche '$BRANCH' introuvable dans le dépôt distant."
    fi
...
sync_repo() {
    if [[ -d "$PROJECT_DIR/.git" ]]; then
        ...
        git -C "$PROJECT_DIR" checkout "$BRANCH" 2>/dev/null \
            || die "Branche '$BRANCH' introuvable dans le dépôt local après fetch."
...
start_server() {
    ...
    printf '\n'
    ok "=== Serveur prêt ==="
    printf '   Branche         : \033[33m%s\033[0m\n'                   "$BRANCH"
